...
<div id="mySidebar" class="sidebar">
  ...
  <a href="ChangePassword.php?id=<?php echo $row["facIndex"];?>">Change Password</a>
     <a href="project-Hod.php?logout='1'">Logout</a>
...
<?php
require 'connect.php';
$stmt=$connect->prepare("select * from uploads");
$stmt->execute();
...
<!--<a href="ForwardFile.php?id=<?php echo $row['id']?>" class="btn btn-primary"id="proceed">proceed to Project in charge</a>-->
<tr>
<?php
}
?>
...
//var id=document.querySelector('.table.td');
...
e.innerHTML= "REJECTED";
 ...
 $.post('updateFile.php',{eid:eid,check:check},function(data)
			{
				$('#demo').html(data);
			}
			);
...
function getComment(e1,e2)	
{
	...
	var getid=e2;
	//console.log(e2);
	$.post('updateFile.php',{comment:comment,getid:getid},
	function(data)
			{
				$('#demo').html(data);
